...
const ProductsTable = ({ variantsWithData }) => {
  ...
  useEffect(() => {
    ...
    variantsWithData.forEach((variant) => {
      initialPriceType[variant.id] = variant.show_units_per === "pallet" ? "pallet" : "pack";
  ...
  const handleUnitChange = async (itemId, variantId, change) => {
    const newUnits = Math.max(0, (itemUnits[itemId] || 0) + change);
    setItemUnits((prev) => ({ ...prev, [itemId]: newUnits }));

    try {
      const pricePer = variantPriceType[variantId] === "pallet" ? "unit" : "pack";
    ...
    if (variant) {
      for (const item of variant.items) {
        if (itemUnits[item.id] > 0) {
          try {
            const apiPricePer = pricePer === "pallet" ? "unit" : "pack";
            ...
            <table className={TableStyles.table}>
              ...
              <tbody>
                {variant.items.map((item) => {
                  // Debug logging
                  console.log(`Item ${item.id} data_entries:`, item.data_entries);
                  return (
                    <tr key={item.id}>
                      ...
                      {variant.tableFields.map((field) => {
                        ...
                        console.log(`Field ${field.id} (${field.name}):`, { itemData });
                        return (
                      ...
                      {pricingTiers.map((tier) => {
                        ...
                        return (
                          <td key={tier.id} className="c3">
                            {pricingData ? `$${Number(pricingData.price).toFixed(2)}` : "-"}
                          </td>
                        );
                      })}
